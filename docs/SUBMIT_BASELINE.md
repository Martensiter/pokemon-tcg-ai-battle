...
> **状況（追記）**：初回ベースライン提出は **実施済み**（実測 567.5）。本手順は今後の
> **再提出／別マシンからの提出** 用に残す。以下は初回提出時の背景・手順。

> なぜ今これをやるか：まだ誰も提出しておらず「在席」する価値が高い。既知動作の bundle で低リスク
> （**初回提出の実測 567.5**。過去の自己申告 v2=1127.8 / baseline=948 は小サンプル・別メタ・未検証なので参考程度）。
